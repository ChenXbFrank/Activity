...
import com.activity.common.model.scratchCard.base.BaseScratchCard;
...
public class ScratchCard extends BaseScratchCard<ScratchCard> {
	public static final ScratchCard dao = new ScratchCard();
	
	public ScratchCard mapping(Record cord, ScratchCard info) {
		...
		info.setScanNumber(cord.getStr("SCAN_NUMBER"));
	...
	public Map<String, Object> paginate(int pageNumber, int pageSize,String activityName,String state,String startTime,String endTime,SystemUser user) {
		...
		String sql=" from TB_SCRATCHCARD where 1=1 ";
		// 这里是模糊查询名字
		...
		Page<ScratchCard> page = paginate(pageNumber, pageSize, "select *", sql+" order by CREATE_TIME desc");
		for (int i = 0; i < page.getList().size(); i++) {
			Map<String, Object> model = new HashMap<String, Object>();
			ScratchCard sb = page.getList().get(i);
			model.put("id", sb.getScratchcardId());
