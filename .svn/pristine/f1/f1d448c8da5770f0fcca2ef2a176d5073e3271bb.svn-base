...
//看图时间验证
function lookTimeValidate(){
	if($("#look-time").val()==""){
		$(".lookTime-volidate").show();
	} else{
		$(".lookTime-volidate").hide();
	}
}
//拼图时间验证
function useTimeValidate(){
	if($("#use-time").val()==""){
		$(".useTime-volidate").show();
	} else{
		$(".useTime-volidate").hide();
	}
}
//时间验证
...
$("#award-btn").on("click",function(){
	...
	if(otherProductCheckedLength > 0) {
		...
		award.boxCount=(parseInt($("#sum").html())/parseInt(award.spec)).toFixed(2);
