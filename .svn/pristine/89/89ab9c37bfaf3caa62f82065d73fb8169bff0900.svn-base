			...
			//开酒瓶活动
				...
				if(!StringUtils.isEmpty(winingId)){
					...
					for (OpenWineWxUserWinning openWineWxUserWinning : winInfoList) {
						...
						so.setCreateTime(openWineWxUserWinning.getCreateTime());
						//先判断中的是什么类型  奖品类型1=实物 2=积分  3=红包   4=谢谢惠顾
						String prizeType = openWineWxUserWinning.getPrizeType();
						so.setPrizeName(openWineWxUserWinning.getPrizeProject());
						if(prizeType.equals("1")){
							so.setWinType("实物");
							so.setWinInfo("实物奖品");
						}else if(prizeType.equals("2")){
							so.setWinType("积分");
							so.setWinInfo(openWineWxUserWinning.getWinningMoney());
						}else if(prizeType.equals("3")){
							so.setWinType("红包");
							so.setWinInfo(openWineWxUserWinning.getWinningMoney());
						}else if(prizeType.equals("4")){
						...
						so.setWinTime(openWineWxUserWinning.getCreateTime());
						so.setRecvierName(openWineWxUserWinning.getAddresseeName());
						so.setRecvierPhone(openWineWxUserWinning.getAddresseePhone());
						so.setRecvierAddress(openWineWxUserWinning.getAddresseeAddress());
						so.setSender(openWineWxUserWinning.getLssueUserName());
						so.setCompany(openWineWxUserWinning.getDeliverCompany());
						so.setOrder(openWineWxUserWinning.getDeliverNumber());
						so.setState(openWineWxUserWinning.getState());
						result.add(so);
					}
				}
			}
			//大光瓶抢红包活动
			else if(mark.equals(PropKit.use("system.properties").get("JzRed"))){
				String activityName = "";
				if(!StringUtils.isEmpty(activityWxId)){
					JzRedWxUser jzRedWxUser = JzRedWxUser.dao.findById(activityWxId);
					String jzRedId = jzRedWxUser.getJzRedId();
					JzRed jzRed = JzRed.dao.findById(jzRedId);
					activityName = jzRed.getActivityName();
					so.setCreateTime(jzRedWxUser.getCreateTime());
				}
				if(!StringUtils.isEmpty(winingId)){
					List<JzRedWxUserWining> jzRedWinList = jzRedService.queryJzRedWxUserWiningByWxUserId(activityWxId);
					for (JzRedWxUserWining jzRedWxUserWining : jzRedWinList) {
						so.setActivityName(activityName);
						so.setTypeName("大光瓶抢红包活动");
						so.setPrizeName(jzRedWxUserWining.getPrizeProject());
						if(jzRedWxUserWining.getPrizeProject().equals("谢谢惠顾")){
							so.setWinType("谢谢惠顾");
						}else{
							so.setWinType("积分/红包");	
						}
						so.setWinInfo(jzRedWxUserWining.getWinningMoney());
						so.setWinTime(jzRedWxUserWining.getCreateTime());
						result.add(so);
					}
				}
			}
