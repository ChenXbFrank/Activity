...
if(volidSuccess){
	...
	if(isClickWhere){//修改里的保存
		...
		$(".daqian").html(checkMoney1.toFixed(2))
	}else{
		...
		if($("#bargaining-situation").val()=="谢谢惠顾"){
			...
			$(".daqian").html(checkMoney2.toFixed(2))
	}
	createTableOtherNew(addAwards.length,addAwards);
	awardValidate();
...
if(volidSuccess){
	...
	if(isClickWhere1){
		addAwards1[index1].type=$("#bargaining-situation1").val();
		addAwards1[index1].name=$("#proName1").val();
		addAwards1[index1].number=$("#proNum1").val();
		addAwards1[index1].probability=$("#probability1").html();
		var awardType=addAwards1[index1].type;
		if(awardType=="红包"){
			addAwards1[index1].money=$("#money1").val();
		}else if(awardType=="积分"){
			addAwards1[index1].integral=$("#integral1").val();;
		}else if(awardType=="实物"){
			addAwards1[index1].pic1Src=$("#pic1").attr("src");
		}else if(awardType=="谢谢惠顾"){
			addAwards1[index1].money='0';
			addAwards1[index1].name='谢谢惠顾';
		}
		createTableOtherNew1(addAwards1.length,addAwards1);
		var checkedNumber7=0;
		var checkMoney7=0;
		for(var k in addAwards1){
			checkedNumber7+=parseFloat(addAwards1[k].number);
			checkMoney7+=parseFloat(addAwards1[k].money)*parseFloat(addAwards1[k].number);
		}
		$(".daqian1").html(checkMoney7.toFixed(2))
	}else{
		var addAwardObj1=new Object();
		addAwardObj1.type=$("#bargaining-situation1").val();
		if($("#bargaining-situation1").val()=="谢谢惠顾"){
			addAwardObj1.name="谢谢惠顾";
			addAwardObj1.pic1Src="";
			addAwardObj1.money="0";
			addAwardObj1.integral="";
		}else{
			if($("#bargaining-situation1").val()=="实物"){
				addAwardObj1.pic1Src=$("#pic1").attr("src");
				addAwardObj1.money="";
				addAwardObj1.integral="";
			}else if($("#bargaining-situation1").val()=="红包"){
				addAwardObj1.money=$("#money1").val();
				addAwardObj1.pic1Src=" ";
				addAwardObj1.integral="0";
				addAwardObj1.name=$("#proName1").val();
			}else if($("#bargaining-situation1").val()=="积分"){
				addAwardObj1.integral=$("#integral1").val();
				addAwardObj1.pic1Src=" ";
				addAwardObj1.money="0";
			}
			addAwardObj1.name=$("#proName1").val();
			...
			addAwardObj1.number=$("#proNum1").val();
			addAwardObj1.probability=$("#probability1").html();
			addAwards1.push(addAwardObj1);
			
			var checkedNumber8=0;
			var checkMoney8=0;
			for(var p in addAwards1){
				checkedNumber8+=parseFloat(addAwards1[p].number);
				checkMoney8+=parseFloat(addAwards1[p].money)*parseFloat(addAwards1[p].number); 
			}
			//alert(addAwards1[p].money)
			$(".daqian1").html(checkMoney8.toFixed(2))
	}
...
function deleteOtherProduct(obj){
	...
	$(".daqian").html(checkMoney4.toFixed(2))
}
...
function deleteOtherProduct1(obj){
	...
	$(".daqian1").html(checkMoney5.toFixed(2))
	console.log(addAwards1);
	awardValidate1();
}
/*点击修改进行的事件*/
...
function copyAwardSet(obj){
	...
	var awardNumChecked=$("input[name='awardNum']:checked").val();
		...
		if(residueAwardNum<0){
			...
			$(".daqian").html(checkMoney3.toFixed(2))
		}
...
function copyAwardSet1(obj){
	...
	var awardNumChecked=$("input[name='awardNum']:checked").val();
		...
		if(residueAwardNum1<0){
			...
			$(".daqian1").html(checkMoney6.toFixed(2))
		}
