	...
	function modify(obj) {
		...
		var awardpattern =addAwards[index].Pattern;
		var frame_value =addAwards[index].frame_value;
		...
		if(awardType == "红包") {
			...
			$('#bargaining-situations').val(frame_value)
			if(awardpattern==1 ){
				//alert("第一个")
	...
	function awardSetTotal() {
		...
		if(hasSetAwardNum == awardSetNum) {
			...
			/*else if(awardSum>parseInt($("#awardTotal").html())){
				$(".tishi4").show();
