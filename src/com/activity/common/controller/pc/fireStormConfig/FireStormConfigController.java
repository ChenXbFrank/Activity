package com.activity.common.controller.pc.fireStormConfig;
 ...
 * 2018年7月3日
 */
@ControllerPath(controllerKey = "/fireStormConfig")
public class FireStormConfigController extends BaseController {
	// 运单Service
	...
	public void robRedConfigUpdate() {
		...
		render("/fireStormConfig/fireStorm-edit.html");
	}
	...
	public void robRedConfigSave(){
		...
		TbCustomActivity fireActivityByRemake = fireStormService.getFireActivityByRemake("fireStorm");
		FireStormRed red=new FireStormRed();
				...
				if("积分".equals(prize.getPrize())){
					if(prize.getMoney()!=null||!prize.getMoney().equals("")){
						if(Integer.parseInt(prize.getMoney())!=0){
							FireStormRedPrize p=new FireStormRedPrize();
		...
		result = red.set("ID", activityId)
				   ...
				   .set("BACKGROUND_IMG", fireActivityByRemake.getImg())
		...
		if(result){
			...
			for (SellerOutInfoChoice so : sellerOutList) {
				...
				as.set("ID", UUID.randomUUID().toString().replaceAll("-", ""))
				  ...
				  // 状态 1可以用 2不可用
				  .set("STATE", 2)
				  .set("BELONG_ACTIVITY", PropKit.use("system.properties").get("FireStorm"))
		...
		new LogInfo().set("ID", newId()).set("USE_USERCODE", user.getUserCode()).set("USE_FUNCTION", "保存-火爆小酒抢红包活动")
		.set("USE_TIME", uitils.newDate()).save();
		renderJson(map);
	}
	...
	public void allMyActvity(){
		...
		SystemUser user = (SystemUser) getSession().getAttribute("loginUser");
		List<FireStormRed> robredList = fireStormService.getFireStormRedByUserCode(user.getUserCode(),activityName,state);
		List<IntegralExchangeActivity> activityList = new ArrayList<IntegralExchangeActivity>();
		for (FireStormRed rr : robredList) {
			IntegralExchangeActivity ba = new IntegralExchangeActivity();
			ba.setActivityId(rr.getId());
		...
		new LogInfo().set("ID", newId()).set("USE_USERCODE", user.getUserCode()).set("USE_FUNCTION", "查询-火爆小酒-抢红包活动-我的活动查询")
		.set("USE_TIME", uitils.newDate()).save();
		render("/fireStormConfig/fireStorm-my-all-activity.html");
	}
	...
	public void robRedInfo(){
		...
		FireStormRed rr = FireStormRed.dao.findById(redRedId);
		// 根据活动ID查询该活动的相关的信息
		List<ActivitySellerOutInfo> sellerInfoList = sellerOutInfoService.getSellerInfoByActivityId(redRedId);
		//奖品信息
		List<FireStormRedPrize> rrpList = fireStormService.getFsPrizeByActivityId(redRedId);
		setAttr("robRed", rr);
		...
		.set("USE_TIME", uitils.newDate()).save();
		render("/fireStormConfig/fireStorm-my-one-activity-info.html");
	}
	...
	public void redRobReport1(){
		...
		List<FireStormRed> robRedList = fireStormService.getAllFireStormRedByUserCode(userCode, activityName, result, startTime, endTime);
		...
		render("/fireStormConfig/fireStorm-activity-report-all.html");
	}
	...
	public void singleRedRobReport(){
		...
		List<FireStormRedWxUser> rrwuList = fireStormService.getFireStormRedWxUserByRedId(activityId,nickName);
		...
		.set("USE_TIME", uitils.newDate()).save();
		render("/fireStormConfig/fireStorm-activity-report.html");
	}
	...
	public void redRobReportInfo(){
		...
		String activityId = getPara("activityId");
		//根据用户参与记录ID 查询用户中奖记录
		List<FireStormRedWxUserWining> rrwuwList = fireStormService.queryFireStormRedWxUserWiningByWxUserId(robRedWxUserId);
		...
		setAttr("activityId", activityId);
		setAttr("activityName", activityName);
		render("/fireStormConfig/fireStorm-activity-report-info.html");
	}
	...
	public void redRobApprove(){
		...
		.set("USE_TIME", uitils.newDate()).save();
		render("/fireStormConfig/fireStorm-activity-approve.html");
	}
	...
	public void redRobApproveInfo(){
		...
		.set("USE_TIME", uitils.newDate()).save();
		render("/fireStormConfig/fireStorm-activity-approve-info.html");
	}
	...
	public void saveRedRobApprove(){
		...
		new FireStormRed().set("ID", activityId).set("STATE", state).set("COMFIRMUSERID", user.getUserId()).set("COMFIRMUSERNAME", user.getUserName())
	...
	public void getRebPageInfos() {
		...
		Map<String, Object> map = FireStormRed.dao.paginate(Integer.parseInt(currentPage), page,activityName,state,startTime,endTime,CacheLoginUser());
	...
	public void getRebPageMoreInfos() {
		...
		int page = Integer.parseInt(pageCount);
		String activityId = getPara("activityId");
		FireStormRed robRed = FireStormRed.dao.findById(activityId);
		String nickName = getPara("nickName");
		String currentPage = getPara("currentPage");
		Map<String, Object> model = new HashMap<String, Object>();
		Map<String, Object> map = FireStormRedWxUser.dao.paginate1(Integer.parseInt(currentPage), page,activityId,nickName);
	...
	public void exportRebExcel(){
		...
		// 导出excel的标题
		String title ="火爆小酒红包报表"+DateUitils.newDateByYMD();
		SystemUser user = (SystemUser) getSession().getAttribute("loginUser");
		String userCode = user.getUserCode();
		List<FireStormRed> list=new ArrayList<>();
		if(user.getUserId().intValue()==-1){
			String sql="select * from FIRE_STORM_RED";
		    list = FireStormRed.dao.find(sql);
		}else{
			String sql="select * from FIRE_STORM_RED where ACITIVITY_LAUNCH_USERCODE= ? ";
			list=FireStormRed.dao.find(sql,userCode);
		}
		List<FireStormRed> list1=new ArrayList<>();
		for (FireStormRed robRed : list) {
	...
	public void exportRebInfoExcel(){
		...
		// 导出excel的标题
		String title ="火爆小酒红包详情报表"+DateUitils.newDateByYMD();
		String activityId = getPara("activityId");
		String sql="select * from FIRE_STORM_RED_WXUSER where RED_ID = ? ";
		List<FireStormRedWxUser> list = FireStormRedWxUser.dao.find(sql,activityId);
		List<RedRodWinInfo> list1=new ArrayList<>();
		String sql1=" select * from FIRE_STORM_RED_WXUSER_WINNING where RED_WXUSER_ID = ? ";
		for (FireStormRedWxUser r : list) {
			...
			String redWxuserId = r.getId();
			FireStormRedWxUserWining win = FireStormRedWxUserWining.dao.findFirst(sql1,redWxuserId);
			if(win!=null){
	...
	public void deleteRedActivity(){
		...
		//奖品信息
		List<FireStormRedPrize> rrpList = fireStormService.getFsPrizeByActivityId(activityId);
		//红包配置
		for (FireStormRedPrize robRedPrize : rrpList) {
			result = FireStormRedPrize.dao.deleteById(robRedPrize.getId());
		}
		//这里将扫码记录删除一下
		...
		result = FireStormRed.dao.deleteById(activityId);
		if(result){
			renderJson("result","true");
		}else{
