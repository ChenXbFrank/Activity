...
public class WxConfigUitils {
	...
	/**
	 * 获取公众号用户信息
	 * {"subscribe":0,"openid":"o2bUJwDOGGgcm7NvTi2zGjcWnbxo","tagid_list":[]}  未关注
	 * 
	 * {"subscribe":1,"openid":"ousUy1jEltkQM8fl1G_SlSf6sVn8","nickname":"唐","sex":1,
	 * "language":"zh_CN","city":"广元","province":"四川","country":"中国",
	 * "headimgurl":"http:\/\/thirdwx.qlogo.cn\/mmopen\/3yfyFZ5jmiatTyalRynBKCwY4YLj0xDwvruFtaNibJgXDt5yhw7Vywc6VloLdotSOfJnia2ME5dIYZRbbsvmliabVYgjse8jYMDP\/132",
	 * "subscribe_time":1533055965,"remark":"",
	 * "groupid":0,"tagid_list":[],
	 * "subscribe_scene":"ADD_SCENE_QR_CODE","qr_scene":0,"qr_scene_str":""}
	 */
	public synchronized WxUserInfo getPublicUser(String openid,String appId,String appsecret) {
		ApiResult userInfo = UserApi.getUserInfo(openid);
		log.error("***获取公众号用户信息userInfo=====>"+userInfo.getJson().toString());
		WxUserInfo wxuser = new WxUserInfo();
		//这里先判断该用户是否关注了
		log.error("***关注的用户信息转JSONObject--赋值***");
		if(userInfo.get("subscribe").toString().equals("1")){
			wxuser.setSubscribe(userInfo.get("subscribe").toString());
			wxuser.setOpenid(userInfo.get("openid").toString());
			wxuser.setNickname(userInfo.get("nickname").toString());
			wxuser.setSex(userInfo.get("sex").toString());
			wxuser.setLanguage(userInfo.get("language").toString());
			wxuser.setCity(userInfo.get("city").toString());
			wxuser.setProvince(userInfo.get("province").toString());
			wxuser.setCountry(userInfo.get("country").toString());
			wxuser.setHeadimgurl(userInfo.get("headimgurl").toString());
			wxuser.setSubscribe_time(userInfo.get("subscribe_time").toString());
			wxuser.setRemark(userInfo.get("remark").toString());
			wxuser.setGroupid(userInfo.get("groupid").toString());
			wxuser.setSubscribe_scene(userInfo.get("subscribe_scene").toString());
			wxuser.setQr_scene(userInfo.get("qr_scene").toString());
			wxuser.setQr_scene_str(userInfo.get("qr_scene_str").toString());
		}else{
			wxuser.setSubscribe(userInfo.get("subscribe").toString());
			wxuser.setOpenid(userInfo.get("openid").toString());
		}
	...
	/*public synchronized WxUserInfo getPublicUser(String openid,String appId,String appsecret) {
		ApiResult userInfo = UserApi.getUserInfo(openid);
		log.error("***获取公众号用户信息userInfo=====>"+userInfo.getJson().toString());
		log.error("***获取公众号用户信息openid***>"+openid);
		log.error("***获取公众号用户信息appId***>"+appId+"***appsecret***"+appsecret);
		String accessToken = AccessTokenUtils.getAccessToken(appId, appsecret);
		log.error("***获取新的token***"+accessToken);
		// 读取用户基本信息  这是获取关注用户的信息
		String token = HttpUtils.get("https://api.weixin.qq.com/cgi-bin/user/info?access_token=" + accessToken
				+ "&openid=" + openid + "&lang=zh_CN");
		log.error("***读取用户基本信息token***"+token);
		JSONObject s = JSONObject.fromObject(token);
		log.error("***关注的用户信息转JSONObject--userInfoJson***"+s);
		WxUserInfo wxuser = new WxUserInfo();
		//这里先判断该用户是否关注了
		log.error("***关注的用户信息转JSONObject--赋值***");
		if(s.get("subscribe").toString().equals("1")){
