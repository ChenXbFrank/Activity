...
import com.activity.common.model.turntable.base.BaseTurnTableWxUser;
...
public class TurnTableWxUser extends BaseTurnTableWxUser<TurnTableWxUser>{
	public static final TurnTableWxUser dao = new TurnTableWxUser();
	
	public TurnTableWxUser mapping(Record cord, TurnTableWxUser info) {
		info.setTurntableWxuserId(cord.getStr("TURNTABLE_WXUSER_ID"));
	...
	public Map<String, Object> paginate(int pageNumber, int pageSize,String activityId, String nickName,String state,String goodsState) {
		...
		String sql=" from TB_TURNTABLE_WXUSER where 1=1 ";
		if (!"".equals(activityId)&&activityId!=null) {
			sql += " and TURNTABLE_ID = '" + activityId+"'";
		}
		// 这里是模糊查询名字
		...
		Page<TurnTableWxUser> page = paginate(pageNumber, pageSize, "select *", sql+" order by CREATE_TIME desc");
		for (int i = 0; i < page.getList().size(); i++) {
			Map<String, Object> model = new HashMap<String, Object>();
			TurnTableWxUser sb = page.getList().get(i);
			model.put("id", sb.getTurntableWxuserId());
			model.put("turnid", sb.getTurntableId());
